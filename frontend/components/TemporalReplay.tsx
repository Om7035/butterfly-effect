...
interface PredictabilityHorizon {
  hours: number;
  label: string;
  warning: string;
  max_hop?: number;
...
interface Props {
  ...
  predictabilityHorizon?: PredictabilityHorizon;
}

export default function TemporalReplay({ prefersReduced, predictabilityHorizon }: Props) {
  const [time, setTime] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [speed, setSpeed] = useState<Speed>(1);
  const [showSpeedMenu, setShowSpeedMenu] = useState(false);
  const [showHorizonTip, setShowHorizonTip] = useState(false);
  const trackRef = useRef<HTMLDivElement>(null);
  const rafRef = useRef<number | null>(null);
  const lastTickRef = useRef<number>(0);

  const horizonHours = predictabilityHorizon?.hours ?? MAX_HOURS;
  const horizonPct = (horizonHours / MAX_HOURS) * 100;
  const isPastHorizon = time > horizonHours;
  ...
  // Confidence level based on current time vs horizon
  const confidenceLevel = time <= horizonHours * 0.5
    ? { label: "High confidence", color: "#34d399" }
    : time <= horizonHours
    ? { label: "Medium confidence", color: "#60a5fa" }
    : { label: "Low confidence", color: "#fbbf24" };
  ...
  return (
    <div style={{ flexShrink: 0, borderTop: "1px solid rgba(255,255,255,0.05)", background: "rgba(10,14,26,0.95)", padding: "10px 16px" }}>
      ...
      {activeMarkers.length > 1 && (
        <div style={{ display: "flex", gap: "6px", marginBottom: "8px", flexWrap: "wrap", alignItems: "center" }}>
          {activeMarkers.slice(1).map((m) => (
            <motion.span key={m.h} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <motion.span
            key={confidenceLevel.label}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            style={{
              fontSize: "9px", padding: "2px 8px", borderRadius: "20px",
              border: `1px solid ${confidenceLevel.color}44`,
              background: `${confidenceLevel.color}10`,
      ...
      {/* Past-horizon warning */}
      {isPastHorizon && predictabilityHorizon && (
        <motion.div
        ...
        >
          ⚠ Past predictability horizon ({predictabilityHorizon.label}) — {predictabilityHorizon.warning}
        </motion.div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <div style={{ width: "64px", flexShrink: 0 }}>
          <div style={{
            ...
            color: isPastHorizon ? "#fbbf24" : "#7c3aed",
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <div style={{ position: "relative", height: "14px", marginBottom: "2px" }}>
            {MARKERS.map((m) => (
              <span key={m.h} style={{
                position: "absolute", fontSize: "8px",
                color: time >= m.h ? "#475569" : "#2d3748",
                transform: "translateX(-50%)",
                left: `${(m.h / MAX_HOURS) * 100}%`,
                transition: "color 0.3s",
              }}>
                {m.label}
              </span>
            ...
            {/* Horizon label */}
            {horizonHours < MAX_HOURS && (
              <span
                onMouseEnter={() => setShowHorizonTip(true)}
                onMouseLeave={() => setShowHorizonTip(false)}
                style={{
                  position: "absolute", fontSize: "8px", color: "#fbbf24",
                  transform: "translateX(-50%)",
                  left: `${horizonPct}%`,
                  cursor: "help",
                  fontWeight: "700",
              ...
              >
                ...
                {showHorizonTip && (
                  <span style={{
                    ...
                    Predictability horizon — {predictabilityHorizon?.warning}
                  </span>
                )}
              </span>
            )}
          ...
          >
            ...
            <div style={{
              ...
              width: `${pct}%`,
              background: isPastHorizon
                ? "linear-gradient(90deg, #7c3aed, #34d399, #fbbf24)"
                : "linear-gradient(90deg, #7c3aed, #34d399)",
              borderRadius: "3px",
              transition: playing ? "none" : "width 0.1s",
            }} />

            {/* Low-confidence zone — shaded overlay past horizon */}
            {horizonHours < MAX_HOURS && (
              <div style={{
                position: "absolute",
                left: `${horizonPct}%`,
                top: 0,
                height: "100%",
                width: `${100 - horizonPct}%`,
                background: "repeating-linear-gradient(45deg, rgba(251,191,36,0.06) 0px, rgba(251,191,36,0.06) 2px, transparent 2px, transparent 6px)",
            ...
            {MARKERS.slice(1).map((m) => (
              <div key={m.h} style={{
                ...
                left: `${(m.h / MAX_HOURS) * 100}%`,
                background: time >= m.h ? "#34d399" : "#2d3748",
            ...
            <div style={{
              ...
              background: "#0a0e1a",
              border: `2px solid ${isPastHorizon ? "#fbbf24" : "#7c3aed"}`,
              left: `calc(${pct}% - 7px)`,
              boxShadow: `0 0 8px ${isPastHorizon ? "rgba(251,191,36,0.5)" : "rgba(124,58,237,0.5)"}`,
              transition: playing ? "none" : "left 0.1s, border-color 0.3s, box-shadow 0.3s",
            }} />
          </div>
        </div>
            ...
            <div style={{ position: "absolute", bottom: "100%", right: 0, marginBottom: "4px", background: "rgba(15,23,42,0.98)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px", overflow: "hidden", zIndex: 10 }}>
              {SPEEDS.map((s) => (
                <button key={s} onClick={() => { setSpeed(s); setShowSpeedMenu(false); }}
                  style={{ display: "block", width: "100%", padding: "6px 14px", background: s === speed ? "rgba(124,58,237,0.15)" : "none", border: "none", color: s === speed ? "#a78bfa" : "#94a3b8", fontSize: "11px", cursor: "pointer", textAlign: "left", fontFamily: "monospace" }}>
                  {s}x
                </button>
              ))}
            </div>
