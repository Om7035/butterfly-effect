...
function edgeColor(confidence: number): string {
  ...
  if (confidence > 0.6) return '#60a5fa';
...
function CausalEdge({
  ...
  useEffect(() => {
    ...
    if (!pathEl) return;
    const speed = 0.0008 + confidence * 0.0006;
    const tick = () => {
  ...
  return (
    <>
      <path id={`edge-path-${id}`} d={edgePath} fill="none"
        stroke={`${color}18`} strokeWidth={strokeW + 2} />
      ...
      <circle ref={dotRef} r={2.5 + strength * 1.5} fill={color}
